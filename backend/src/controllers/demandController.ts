...
export const demandController = {
  ...
  async list(req: AuthRequest, res: Response) {
    try {
      ...
      const demands = rawDemands.map((d) => {
        const dynamicPriority = calculateDynamicPriority(d.data_entrega, d.dificuldade || 'Médio');
        return {
          ...d,
  ...
  async getById(req: AuthRequest, res: Response) {
    try {
      ...
      const demand = await prisma.tarefa.findFirst({
        where: {
          id_tarefa: Number(id),
          id_usuario: userId,
        },
      ...
      const dynamicPriority = calculateDynamicPriority(demand.data_entrega, demand.dificuldade || 'Médio');
  ...
  async create(req: AuthRequest, res: Response) {
    try {
      ...
      const dynamicPriority = calculateDynamicPriority(data_entrega, diffCalculated);

      const demand = await prisma.tarefa.create({
        data: {
          titulo,
  ...
  async update(req: AuthRequest, res: Response) {
    try {
      ...
      const dynamicPriority = calculateDynamicPriority(newDeadline, newDiff);
  ...
  async toggleStatus(req: AuthRequest, res: Response) {
    try {
      ...
      const dynamicPriority = calculateDynamicPriority(updated.data_entrega, updated.dificuldade || 'Médio');
