    ...
    async getDashboardStats(req, res) {
        try {
            ...
            const allDemands = rawDemands.map((d) => ({
                ...d,
                prioridade: (0, priorityCalculator_1.calculateDynamicPriority)(d.data_entrega, d.dificuldade || 'Médio'),
            }));
            const now = new Date();
            ...
            const marcadores = await prisma_1.prisma.marcador.findMany({
                where: { id_usuario: userId },
                include: {
                    tarefas: true,
                },
            ...
            const avisos = await prisma_1.prisma.notificacao.findMany({
                where: { id_usuario: userId },
