...
exports.eventController = {
    ...
    async list(req, res) {
        try {
            ...
            const events = await prisma_1.prisma.evento.findMany({
                where: whereClause,
    ...
    async create(req, res) {
        try {
            ...
            const event = await prisma_1.prisma.evento.create({
                data: {
                    titulo,
    ...
    async update(req, res) {
        try {
            ...
            const { id } = req.params;
            const { titulo, descricao, data_inicio, data_fim, local } = req.body;
            const existing = await prisma_1.prisma.evento.findFirst({
                where: { id_evento: Number(id), id_usuario: userId },
            });
            if (!existing) {
                return res.status(404).json({ error: 'Evento não encontrado.' });
            }
            const updated = await prisma_1.prisma.evento.update({
    ...
    async delete(req, res) {
        try {
            ...
            await prisma_1.prisma.evento.delete({
                where: { id_evento: Number(id) },
            });
